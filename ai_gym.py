...
for i in range(num_episodes):
    ...
    while True:
        ...
        if done:
            ...
            summary, ll, la, l, _ = sess.run([merged_summary, log_lik, log_lik_adv, loss, train], feed_dict={X: xs,
                                                                                                             Y: ys,
    ...
    # Print status
    print("[Episode {%d:>}] Reward: {reward_sum:>4} Loss: {l:>5.5}" % i)

    if np.mean(EPISODE_100_REWARD_LIST) >= 195.0:
        print("Game Cleared within {%d} steps with the average reward: {np.mean(EPISODE_100_REWARD_LIST)}" % i)
        break
